...
Y = 0 #Poisition en Y de l'affichage du bonus qui est aléatoire également.
...
def Nom(): #Fonction permettant de choisir son nom.
    global nom
    nom = str(input("Com t’agradaria ser cridat? (No més de 10 caràcters): "))
    while len(nom)>10: #Si le nom possède plus de 10 caractères, il faut choisir un autre nom.
        nom = str(input("El nom que heu triat té més de 10 caràcters. Torneu-ho a provar: "))
    return nom
...
#Création des listes :
if langue == 1:
    objet = ["Mina de monedes", "Fabricà de peces", "Excavadora"] #Liste des noms des objets que l'on peut acheter.
elif langue == 2:
    objet = ["", "", ""]
prix = [1, 5, 10, 50] #Liste des prix des objets que l'on peut acheter.
Q = [0, 0, 0] #Quantité d'objets achetés.
texte_objet = [0, 0, 0] #Liste des noms des
clic_par_s = [0.0] #
compteur_s = [0] #

e = [0, 0, 0, 0, 0, 0, 0, 0, 0] #
v = [0, 0, 0, 0, 0, 0, 0, 0, 0] #
t = [POLICE3.render("Recull 100 monedes.", True, BLANC), POLICE3.render("Recull 500 monedes.", True, BLANC), POLICE3.render("Recull 1000 monedes.", True, BLANC),
    POLICE3.render("Tenir un PPS de 5.", True, BLANC), POLICE3.render("Tenir un PPS de 10.", True, BLANC), POLICE3.render("Tenir un PPS de 15.", True, BLANC),
    POLICE3.render("Obteniu 1 plus.", True, BLANC), POLICE3.render("Obteniu 3 plus.", True, BLANC), POLICE3.render("Perdre 100 monedes.", True, BLANC)] #
E = [0, 0, 0, 0, 0, 0, 0, 0, 0] #
...
def Fin():
    ...
    if jeu == 1:
        ...
        mode = "Exit"
    bravo = POLICE.render("Ben fet " + nom, True, BLANC)
    MODE = POLICE.render("Heu acabat Clickos en mode " + mode, True, BLANC)
    temps = POLICE.render("En " + str(int(compteur)) + " segons", True, BLANC)
    if fin == 1:
        fenetre.fill(NOIR)
...
def vitesse():
    ...
    if niveau == 0:
        ...
        texte0 = POLICE2.render("Nivell 1   :   0$", True, NOIR)
        ...
        if niveau != 5:
            texte_objet4 = POLICE2.render("Nivell " + str(int(niveau + 1)) + " " + str(prix[3]) + "$", True, NOIR)
        else:
            texte_objet4 = POLICE2.render("Final  " + str(prix[3]) + " $", True, NOIR)
        texte_nom = POLICE.render(nom + " : nivell " + str(int(niveau)), True, BLANC)
    ...
    if niveau >= 1 and exploit == 0:
        if AIDE == 1:
            ...
            texte_aide = POLICE3.render("+1 nivell", True, BLANC)
...
def succes():
    ...
    if niveau == 0:
        fenetre.blit(bouton_niveau0, bouton_rect[0])
        texte0 = POLICE2.render("Juga   :   0$", True, NOIR)
        fenetre.blit(texte0, (275, 235))
    elif niveau >= 1:
        # Chargement des autres images ne variant pas.
        ...
        if exploit == 0:
            ...
            texte_objet[1] = POLICE4.render(str(int(Q[1])) + "x  " + objet[1] + "  " + str(prix[1]) + "$", True, NOIR)
            texte_objet[2] = POLICE2.render(str(int(Q[2])) + "x  " + objet[2] + "  " + str(prix[2]) + "$", True, NOIR)
            texte_objet4 = POLICE2.render("Llista de gestions", True, NOIR)
            texte_nom = POLICE.render(nom + " : " + str(nb_exploits) + "/9 explotacions aconseguides", True, BLANC)
            compteur_texte = POLICE2.render(str(int(compteur)), True, BLANC)
        ...
        if exploit == 0:
            ...
            for i in range(0, 9):
                if i < 3:
                    fenetre.blit(t[i], (118 + i * 156, 160))
                elif i >= 3 and i < 6:
                    fenetre.blit(t[i], (123 + (i-3) * 156, 280))
                elif i >= 6:
...
while quitter == False:
    ...
    if jeu == 0:
        texte_mode1 = POLICE2.render("MODE VELOCITAT", True, BLANC)
        texte_mode2 = POLICE2.render("MODE EXIT", True, BLANC)
        texte = POLICE.render("Premeu Enter per confirmar", True, (255, 255, 255))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYUP and event.key == K_RETURN:
            if choix == 1:
                ...
                jeu = 2
                choix = 0
        if (event.type == pygame.KEYDOWN and event.key == K_ESCAPE) or event.type == pygame.QUIT:  # Nous récupérons l'événement d'une touche qui est appuyée ET qu'il s'agit de la touche "echap".
